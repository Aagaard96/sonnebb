    ...
    <article className="grid lg:grid-cols-3 place-items-center gap-20 lg:gap-5 max-w-7xl mx-auto">
      {reviews.map((review, i) => (
        <StaggerAnimation key={i} index={i}>
          <Card
            isblurred
            className="max-w-[325px] h-fit md:max-w-[400px] p-5 bg-accentText overflow-visible text-mainText border-1 border-mainText shadow-sm shadow-mainText"
          >
            <CardHeader className="font-bold text-xl">{review.name}</CardHeader>
